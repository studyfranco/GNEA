...
##            text : outdir      Directory where the graphic are saved
## Return : void
volcanoplot3 <- function(tot, cell, lfc, pvalue, outdir){
...
cellcounts <- function(ratioCount, normcounts, dataCountsSCcount, dataCountsReferences, outdir){
  ...
  cellresult2 <- log2((NormmatEnv[,2:ncol(NormmatEnv)])/(NormmatEnv[,1])) # Big modification of lfc for the counts !!!!!! Remove infinite and allow access a number of genes.
...
normalisationMean <- function(dataCountsReferences,dataSumSCcount,dataCountsSCcount,ratioCount,outdirXP,Core){
  ...
  dataCountsReferences <- dataCountsReferences/ncol(ratioCount)
  ### Remove count number
