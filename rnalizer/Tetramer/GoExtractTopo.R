...
FilesFolder <- "/home/studerf/mnt/Zone4/rnalizer/SCNorm/results/Ref.H9_SC.Orga1A_lfc.1_pval.0.01_Norm.Mean/SingleCell/ResultTetra/Reguloms/"
outdir <- "/home/studerf/mnt/Zone2/Pipeline/SCNorm/results/Ref.H9_SC.Orga1A_lfc.1_pval.0.1_Norm.Mean/SingleCell/pval.0.05_lfc.1/ResultTetra/Reguloms"
pval <- 0.01
ncol <- 2
Core <- 6
orga <- "Hs"
...
if (length(listFilesFolder) > 0 ) {
  ...
  ListGoTerm <- foreach(j = 1:length(listFilesFolder), .packages = c("FGNet"), .inorder = FALSE) %dopar% {
    ...
    if(recogPercent >= 0.1){
      ...
      write.table(results, file = file.path(outdirGo,paste("GOTerm_",c(substr(nomfile,coupure,nchar(nomfile)-8)),".tsv",sep = "")), quote = FALSE, sep='\t',row.names=FALSE)
      list(GOresult)
    } else {
      vide <- c(1)
